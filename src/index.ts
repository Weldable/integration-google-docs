  ...
  actions: [
    {
      ...
      name: 'Read document',
      description:
        "Read a Google Doc as markdown. Returns line-numbered output in cat -n style (' 1→# Heading'). The ' N→' prefix is display-only; edit matches against the raw markdown. If endLine < totalLines, paginate with { offset: endLine + 1 }. Default limit is 2000 lines.",
      intents: [
        'read my google doc',
        'open this document',
      ...
      execute: async (args, ctx) => {
        ...
        const doc = res.data as DocsDocument
    ...
    },
    {
      ...
      description:
        "Create a new Google Doc. Pass the body as markdown in the `content` field — it will be rendered with real Docs formatting (headings, bold, italic, inline code, links, bullet/numbered lists, horizontal rules). Tables and images are not supported in v1 and will render as plain text.",
      intents: [
      ...
      execute: async (args, ctx) => {
        ...
        const createdData = created.data as { documentId?: string }
      ...
      execute: async (args, ctx) => {
        ...
        if (count === 0) {
          throw new IntegrationValidationError(
            'edit failed: no match for oldText in document. Read the document first to find the exact text.',
            'oldText',
          )
        }
        if (count > 1) {
          ...
          throw new IntegrationValidationError(
            `edit failed: oldText matched ${count} locations. Narrow the anchor with more surrounding context. Top candidates:\n` +
...
function optionalPositiveInt(v: unknown, defaultValue: number): number {
